...
if __name__ == '__main__':
    ...
    data_path = Path.home()/'group'/'project'/'datasets'/'data'/\
    '220414_dl_grid_84to90_1to45mm.h5'
    freq_path = Path.home()/'group'/'project'/'results'/'mayfly'/\
    '220303_grad_b_frequency_grid_radius_angle.npz'
    signals = mf.data.MFDataset(str(data_path)).data[:]

    signal_metadata = pd.DataFrame(mf.data.MFDataset(str(data_path)).metadata)

    frequencies = np.load(freq_path)['freq'].flatten()
    radii = np.load(freq_path)['radii'].flatten()
    pitch = np.load(freq_path)['angles'].flatten()

    #iradius = 0
    select_rad = [radii[index_bf]]
    select_pitch = pitch
    ...
    for i, pair in enumerate(zip(select_rad, select_pitch)):
    ...
    signals = SortSignals(signals, signal_metadata, select_rad, select_pitch).reshape((select_freq.shape[0], nch, signals.shape[-1]//nch))

    print(signals.shape, select_freq.shape)
    ...
    config = {
            ...
            'time_dependent'/'beamforming_grids'/'radius_pitch_mini_grids',
            'name':f'220307_radius_pitch_grid_offaxis_beamforming_corrected_rad{int(1000*select_rad[0])}mm'
        }

    x_grid, y_grid = np.meshgrid(
        np.linspace(-0.01, 0.01, config['n_gridx']),
        np.linspace(select_rad[0]-0.01,select_rad[0]+0.01, config['n_gridy']),
        )

    coordinates = np.array([x_grid.flatten(), y_grid.flatten()]).T

    summed_signals = ShiftAndSum(config, signals, 12 * select_freq, coordinates)
    
    config['save_path'].mkdir(parents=True, exist_ok=True)
    np.savez(config['save_path']/config['name'], signals=summed_signals, radius=select_rad, pitch=select_pitch)
