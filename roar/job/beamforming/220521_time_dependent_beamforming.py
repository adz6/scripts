...
def SortGradBFreq(data_radius, data_pitch, gradb_radii, gradb_pitch, gradb_freq):
    ...
    for i, radius in enumerate(data_radius):
        ...
        try:
            ...
            pitch_diff = abs(gradb_pitch - data_pitch[i])
            radii_diff = abs(gradb_radii - data_radius[i])

            pitch_min_inds = np.where(pitch_diff == pitch_diff.min())[0]
            radii_min_inds = np.where(radii_diff == radii_diff.min())[0]



            grid_i, grid_j = np.meshgrid(pitch_min_inds, radii_min_inds)

            nearest_gradb_ind = np.where(grid_i.flatten() == grid_j.flatten())[0]

            data_gradb_freq[i] = np.mean(
                gradb_freq[grid_i.flatten()[nearest_gradb_ind]]
                )
...
if __name__ == '__main__':
    ...
    'data'/'220604_dl_grid_85to88_5mm.h5'
    ...
    #print(np.sort(np.unique(data_radius)))
    #print(np.sort(np.unique(gradb_radii)))
    #print(np.sort(np.unique(data_pitch)))
    #print(np.sort(np.unique(gradb_pitch)))
    ...
    select_rad = data_radius_unique[0] # for data with only 1 radius

    select_inds = np.argwhere(data_radius == select_rad).squeeze()
    print(select_inds.size)

    # select data, and frequencies. Split
    # for big data with only 1 radius ###
    select_inds_split = np.array_split(select_inds, 101, axis=0)
    select_inds = select_inds_split[index_bf]
    print(select_inds.size)
    ########################################

    select_data = data[select_inds, :].reshape((select_inds.size, 60, 1*8192))
    ...
    for i in range(len(select_data_split)):
        ...
        config = {
                ...
                'beamforming'/'time_dependent'/'deep_learning',
                'name':f'220605_dl_grid_85to88_5mm_gradb_sum_{index_bf}_{i}'
            }
        ...
        if (config['save_path']/config['name']).exists():
            ...
            summed_signal_grids = ShiftAndSum(
                config,
                select_data_split[i],
                1.2 * select_freq_split[i],
                coordinates
                )

            signals_max = np.argmax(
                np.sum(abs(summed_signal_grids) ** 2, axis=-1),
                axis=-1,
                ).squeeze()

            summed_signals = []

            for j, n in enumerate(signals_max):
                summed_signals.append(summed_signal_grids[j, n, :])
            ...
            np.savez(
                ...
                signals=np.array(summed_signals),
                radius=select_rad,
