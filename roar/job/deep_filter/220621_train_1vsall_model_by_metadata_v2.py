...
def LoadSignal(config, path, samples=8192):
    ...
    if path.name.endswith('.npy'):
        ...
        print(f'The number of unique background signals is {background_data_inds.size}')
        ...
        print(f'The number of background training signals is {train_background.size}')
        print(f'The number of background validation signals is {val_background.size}')
...
def TrainModel(rank, config, train_config, model_config):
    ...
    with torch.no_grad():
        train_data, val_data, n_train, n_val = LoadSignal(config, config['data_path'], samples = train_config['samples'])
...
if __name__ == "__main__":
    ...
    #pitch_splits = np.array_split(np.linspace(86.5, 88.0, 26), 13)
    #select_split = pitch_splits[index]

    target_pitch = np.linspace(86.5, 88.0, 1501)
    ...
    config = {
        ...
        /f'220621_{target_pitch[0]}to{target_pitch[-1]}deg_5mm_model1_1vsall_v5.tar',
