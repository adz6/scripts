...
def LoadSignalEval(config, path, samples=8192):
    ...
    if path.name.endswith('.npy'):
        ...
        number_copies_train = 2

        # we want the meta data to gauge the performance as a function of pitch angle
        meta_data = {}
        ...
        for i in range(number_copies_train):
            ...
            train_pitch.append(
                file['meta']['theta_min'][train_inds]
            )
            train_energy.append(
                file['meta']['energy'][train_inds]
            )
        ...
        #train_pitch.append(file['meta']['theta_min'][train_inds[0:n_diff]])
        #train_energy.append(file['meta']['energy'][train_inds[0:n_diff]])

        train_data = np.concatenate((*train_data,),)
        train_pitch = np.concatenate((*train_pitch,),)
        train_energy = np.concatenate((*train_energy,),)
        ...
        #background_pitch = file['meta']['theta_min'][background_data_inds]
        #background_energy = file['meta']['energy'][background_data_inds]

        #meta_data['background_pitch'] = background_pitch
        #meta_data['background_energy'] = background_energy
        ...
        train_data = np.concatenate(
            (
                train_data, 
                np.zeros(train_data.shape, dtype=np.complex128),
            ),
            axis=0
            )
    ...
    return torch.tensor(train_data), n_train, n_background, meta_data
...
def EvalModel(rank, config, train_config, model_config):
    ...
    with torch.no_grad():
        eval_data, n_target, n_background, meta_data = LoadSignalEval(config, config['eval_data_path'], samples = train_config['samples'])
...
if __name__ == "__main__":
    ...
    config = {
        ...
        ########### CHECKPOINT ################
        'checkpoint': Path(f'{args.checkpoint}'),
        ########### EVAL RESULT ###############
    ...
    if config['checkpoint'].exists():
        model, optimizer, epoch, loss = LoadCheckpoint(config)
        train_config['model'] = model.eval()
