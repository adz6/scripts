...
parser.add_argument('--prunes', dest='prunes', action='store', default=1)
...
def LoadSignalTrain(config, path, samples=8192):
    ...
    if path.name.endswith('.npy'):
        ...
        number_copies_train = 2 #number_copies_train = (2 * train_background_inds.size) // train_inds.size
        number_copies_val = 2 #number_copies_val = (2 * val_background_inds.size) // val_inds.size
        ...
        # concatenate an array of zeros of equal size to the training data. This will be all noise.
        train_data = np.concatenate(
            (train_data, np.zeros((train_data.shape[0], samples), dtype=np.complex128)),
            axis=0
            )
        ...
        val_data = np.concatenate(
            (val_data, np.zeros((val_data.shape[0], samples), dtype=np.complex128)),
            axis=0
            )
...
def TrainModel(rank, config, train_config, model_config):
    ...
    for iter in range(train_config['prunes']):
        ## train ##
        ...
        config['checkpoint'] = Path(str(config['checkpoint']).split('.')[0]\
        + f'_prune{iter}.' + str(config['checkpoint']).split('.')[-1])
