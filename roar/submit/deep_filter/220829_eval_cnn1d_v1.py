...
def make_command(**kwargs):
    ...
    io_opt = f'--data {kwargs["data"]}' + f' --name {kwargs["name"]}' + f' --checkpoint {kwargs["checkpoint"]}'
    ...
    return python_cmd+' '+train_opt+' '+data_opt+' '+io_opt+' '+output
...
if __name__=='__main__':
    ...
    config = {
        'train_dir': str(output_repo/'220929_eval_cnn1d'),
        'name':'model1',
        'checkpoint': str(output_repo/'220901_train_cnn1d'/'model1'/'model1.tar'),
        'script': str(script_repo/'machine_learning'/'220829_eval_cnn1d_model_v1.py'), # the evaluation script
        'time_string':'7:00:00',
        'ngpu': 1,
        'gpu_type': 'gc_v100s',
        'mem': 200,

        'data': str(data_repo/'220901_dl_test_data_85to90deg_5mm.h5'),
        'pitch_min': 86.0,
        'pitch_max': 88.6,
        'batchsize': 2500,
        'epochs':100,
